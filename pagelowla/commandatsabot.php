...
if (isset($_SESSION['utilisateurs'])) {
  ...
  $pointure = isset($_GET['pointure']) ? $_GET['pointure'] : "";
  ...
  if ($nomC == "all") {
      $requetePropre = "SELECT p.id_produit,c.etat_comm,c.adresse_comm,c.pays_comm,p.pointure, p.nom_produit,c.ville_comm, c.id_commande,p.catégorie_produit,c.numero_telephone, c.nom_proprietaire_commande, p.prix_produit , c.date_commande, c.total_commande
    FROM produits p, commandes c WHERE p.id_produit = c.id_produit AND p.catégorie_produit = 'Sabots' AND  c.nom_proprietaire_commande LIKE '%$NomPropre%' AND c.etat_comm LIKE '%$etat_comm%' AND p.nom_produit LIKE '%$NomP%'  order by id_commande";
  } elseif
  ($NomP != " " and $NomPropre != " " and $etat_comm != " ") {
      $requetePropre = "SELECT p.id_produit, c.etat_comm,c.adresse_comm,p.nom_produit,p.pointure,c.pays_comm,p.catégorie_produit,c.ville_comm,c.numero_telephone, c.id_commande, c.nom_proprietaire_commande,p.prix_produit, c.date_commande, c.total_commande
    FROM produits p, commandes c WHERE p.id_produit = c.id_produit AND p.catégorie_produit = 'Sabots' AND  p.catégorie_produit = '$nomC' ";
  } else {
      $requetePropre = "SELECT p.id_produit,c.etat_comm,c.adresse_comm, c.id_commande,p.pointure,c.pays_comm,p.catégorie_produit,c.ville_comm,c.nom_proprietaire_commande,p.prix_produit , c.numero_telephone, c.date_commande, c.total_commande
    FROM produits p, commandes c WHERE p.id_produit = c.id_produit AND p.catégorie_produit = 'Sabots' AND c.nom_proprietaire_commande LIKE '%$NomPropre%' AND p.catégorie_produit = '$nomC' AND c.etat_comm LIKE '%$etat_comm%' AND p.nom_produit LIKE '%$NomP%' order by id_commande";
  }
...
<li class="nav-item">
  ...
  <ul id="commandes-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
    ...
    <li>
      <a href="commandatsabot.php" class="active">
      ...
      <div class="panel panel-success margetop">
          ...
          <div class="panel-body">
            ...
            <div class="card top-selling overflow-auto" >   
              ...
              <?php
  $requetepl = "SELECT COUNT(c.id_commande) AS count_commandes,p.catégorie_produit FROM commandes c , produits p WHERE c.id_produit=p.id_produit AND p.catégorie_produit = 'Sabots'";
...
<div class="panel-heading">
  ...
  <h7 class="btn btn-primary">
    ...
    <h7 class="badge"><?php echo $commandes['count_commandes']; ?></h7>
  </h7>
</div>
      <div class="panel-body">
              ...
              <tbody>
                  ...
                  <?php while ($commandes = $resultatComm->fetch()) { ?>
                      <tr>
                          ...
                          <?php if ($_SESSION['utilisateurs']['role_user'] == 'ADMIN') { ?>
                              <td>
                                  <button type="submit" style="background-color:purple;" class="btn"><a style="color:white;"  class="downloadlink"
                              ...
                              <td>
                                  <a href="../pages/etttttcommande.php?idCoum=<?php echo $commandes['id_commande'] ?>">
                                      <span class="btn btn-info" >✓</span></a>



                                  <a href="../pages/rrrrrcommande.php?idCoum=<?php echo $commandes['id_commande'] ?>">
                                  <span class="btn btn-danger"  >X</span></a>



                                  <a href="../pages/injoignablecommande.php?idCoum=<?php echo $commandes['id_commande'] ?>">
                                  <span class="btn btn-warning" >📵</span></a>
                                  <a href="../pages/confirmelivrecommande.php?idCoum=<?php echo $commandes['id_commande'] ?>">

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ville_comm = isset($_POST['ville_comm']) ? $_POST['ville_comm'] : "";
    $pays_comm = isset($_POST['pays_comm']) ? $_POST['pays_comm'] : "";
    $pontura = isset($_POST['pontura']) ? $_POST['pontura'] : "";

    // Fetch distinct pointure from produits table
    $stmt_pointure = $pdo->prepare("SELECT DISTINCT pointure FROM produits");
    $stmt_pointure->execute();
    $resultatpointure = $stmt_pointure->fetchAll(PDO::FETCH_ASSOC);

    // Insert commandes data into the table
    $stmt = $pdo->prepare("INSERT INTO commandes(id_commande, etat_comm, pays_comm, ville_comm, nom_proprietaire_commande, adresse_comm, total_commande, numero_telephone, id_produit) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$idP, $etat_comm, $pays_comm, $ville_comm, $NomProre, $adresse_comm, $Tcomm, $Ntele, $idP]);

    // Redirect to the commandatsabot page
    header('Location: commandatsabot.php');
    exit;
}
?>
<?php
...
<head>
  ...
  <title> Prixvado / Nouvelle Commande</title>
        ...
        <div class="panel-body">
          <form method="post" action="">
            <div class="form-group">
              <label for="idP">l'id du sabot :</label>
              <input type="hidden" value="" name="idP" class="form-control" readonly><?php echo $idP ?>
            </div>
            <br>
            <div class="form-group">
              <label for="NomP">le nom de sabot :</label>
              <input disabled type="text" value="<?php echo $NomP ?>" name="NomP" class="form-control">
            </div>
            <br>
            ...
            <div class="form-group">
              ...
              <input type="text" name="pays_comm" placeholder="Entrer le pays" class="form-control">
            </div>
            <br>
            <div class="form-group">
              <label for="ville_comm">Ville :</label>
              <input type="text" name="ville_comm" placeholder="Entrer la ville" class="form-control">
            </div>
            <br>
            ...
            <br>
            <div class="btn btn-warning frer " id="decrease" onclick="decreaseValue()" value="Decrease Value">-</div>
            &nbsp; <input class="fo " type="text" name="Tcomm" placeholder="Entrer le totale" id="number" value="1" />
            &nbsp;
            <div class="btn btn-warning frer " id="increase" onclick="increaseValue()" value="Increase Value">+</div>
            ...
            <div class="form-group">
              <br>
              <label for="pointure">Pointure Disponible : &nbsp
                <?php echo $pointure ?></label>
              <input type="text" name="pontura" placeholder="Entrer une pointure parmi les disponibles" class="form-control">
            </div>
